...
Снимок Beads от 2026-09-26 14:06 UTC (17:06 МСК). Решение по этой сверке: `tgsum-9x6`.
...
- Из 106 задач: 6 выполнены, 1 в работе, 99 открыты. Все 12 эпиков остаются незавершёнными.
...
- В `tgsum-hzm` завершены 6 из 13 задач: `hzm.1` (Project store), `hzm.2` (canonical schema), `hzm.3` (scope, период, delta), `hzm.4` (интерфейсы источников), `hzm.5` (минимальный sanitizer) и `tgsum-ax6` (evidence и Export only bundle).
- Последняя реализация — commit `f97a53f`: Review и Export only проверены на synthetic данных через настоящий Linux Tauri UI; в Beads сохранён результат полного gate — 113 тестов. Это не квалификация настоящего AI-агента или messenger account.
- `hzm.6` (изолированный runner) в работе: начат research, код runner ещё не реализован. Проверенные adapters Codex/Claude, recipes, onboarding и отдельные E2E/adversarial suites остаются открыты.
- Остальные 99 задач открыты. Все 12 эпиков незавершены. Закрытый `tgsum-d9b` — предшествующая основа, отдельно от этих 106 задач.
...
| --- | --- | --- | ---: | ---: | ---: | ---: |
| [tgsum-hzm](#tgsum-hzm) | v0.3 — Context Engine, Projects и первые агенты | В работе | 13 | 6 | 2 | 0 |
