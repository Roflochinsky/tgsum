...
- **`src-tauri/ui/`** — static HTML/CSS/JS (no framework, no build step), served from the app bundle. The design follows Van Gogh's *Wheat Field with Cypresses* (1889, public domain): the painting fills the window behind paper panels, Caveat headings and Alegreya Sans text (OFL, bundled with Cyrillic); the OS dark theme turns it into the same field at night.
